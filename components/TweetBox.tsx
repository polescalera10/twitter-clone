...
function Tweetbox({ setTweets }: Props) {
  ...
  return (
    <div className='flex space-x-2 p-5'>
      <img src={session?.user?.image || "https://links.papareact.com/gll"} className='h-14 w-14 rounded-full object-cover mt-4' alt="" />
      <div className='flex flex-1 items-center pl-2'>
        <form className='flex flex-1 flex-col'>
          ...
          <div className='flex items-center'>
            ...
            <button onClick={handleSubmit} disabled={!input || !session} className='bg-twitter rounded-full px-5 py-2 font-bold text-white disabled:opacity-40'>Tweet</button>
          </div>
          {imageUrlBoxIsOpen && (
          ...
          {image && <img src={image} className='mt-10 h-40 w-full rounded-xl object-contain shadow-lg' />}
        </form>
      </div>
    </div>
  )
